...
// Created: 16-10-2013
// Last Update: Mon 15 Sep 2014 14:37:50 BST
...
void SinglePropBase::loadWithSentence(const Corpus& t, int i) {
  corpus_ = &t; // link to corpus
  id = i;
  // words_ = &t[i];
  // instance_ = &t[i]; //static_cast<const Sentence&>(t);
  sent_length = t[i].size(); //int(instance_->words.size());
  nodes_length = sent_length;//int(instance_->rule.size());
  lbl_error_ = 0.0;
  rae_error_ = 0.0;
...
void SinglePropBase::forwardPropagate(bool autoencode) {
  // Remove and push into more complex class!
  /*
