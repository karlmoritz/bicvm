 ...
 ***************************************************************************/
...

  {
    ...
    assert(!input.empty());
    load_qq::load_file(modelA.corpus, modelB.corpus, input, create_new_dict_A,
                       create_new_dict_B, sennaA, sennaA);
    ...
    if (create_new_dict_B) {
      raeB.createTheta(true); }// deB->createTheta(true); sennaA.applyEmbeddings(); }
  }
  ...
  modelA.trainer = new OpenQATrainer();
